...
//        Toast.makeText(this, radioButton.getText(), Toast.LENGTH_SHORT).show();
    ...
    public void increment(View view){
        if (cylinderSize2.getText().length() == 0){
            ...
            Log.e(TAG, "increment: : "+intCylinderSize2 );
    ...
    private void display(Float number){
        ...
        switch (radioButton.getText().toString()) {
            case "Home Service":
                number *= Float.parseFloat(homePriceFirebase);
                break;
            case "Come to our office":
                number *= Float.parseFloat(officePriceFirebase);
                break;
            default:
                return;
        }
        ...
        table_gas_price.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.child("Price").exists()){
                    Log.e(TAG, "onDataChange: price exists" );
                    mDialog.dismiss();
